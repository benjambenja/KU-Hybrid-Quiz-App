    ...
    $scope.submitSignUp = function(event) {
        ...
        if ($scope.signUpUsername !== undefined && $scope.signUpPassword !== undefined && $scope.confirmPassword !== undefined && $scope.signUpKuId !== undefined) {
            ...
            if ($scope.signUpPassword === $scope.confirmPassword) {
                try {
                    $http.get('http://129.237.121.131:3000/instructor?username=' + $scope.signUpUsername).then(function(response) {
                        ...
                        if ($scope.foundAccount.username === $scope.signUpUsername) {
                            ...
                            $scope.global = Global;
                            $http.get('http://129.237.121.131:3000/instructor').then(function(response) {
                               $scope.global.instructors = response.data;
                                ...
                                $http.post('http://129.237.121.131:3000/instructor', JSON.stringify($scope.dataObj));
...
app.controller('LoginCtrl', function($scope, $location, $http, $mdDialog, Global) {
    $scope.submitLogin = function(event) {
        ...
        if ($scope.loginUsername !== undefined && $scope.loginPassword !== undefined) {
            // checks for account
            try {
                $http.get('http://129.237.121.131:3000/instructor?username=' + $scope.loginUsername + '&password=' + $scope.loginPassword, false).then(function(response) {
...
app.controller('CourseCtrl', function($scope, $http, $location, $mdDialog, Global) {
    ...
    $scope.global.instructor = JSON.parse(localStorage.getItem("instructor"));
    // GETTING COURSE DATA
    $http.get("http://129.237.121.131:3000/course?instructorId=" + $scope.global.instructor[0].instructorId).then(function(response) {
        $scope.global = Global;
...
app.controller('QuizCtrl', function($scope, $http, $location, $mdDialog, Global) {
    ...
    $http.get("http://129.237.121.131:3000/quiz?courseId=" + $scope.global.currentCourseId).then(function(response) {
        $scope.global = Global;
    ...
    // GETTING ROSTER DATA
    $http.get("http://129.237.121.131:3000/roster?courseId=" + $scope.global.currentCourseId).then(function(response) {
        $scope.global = Global;
    ...
    $scope.uploadRoster = function() {
        for (var i = 0; i < $scope.global.rosterUpload.length; i++) {
            $http.post("http://129.237.121.131:3000/roster", JSON.stringify($scope.global.rosterUpload[i])).then(function(reponse) {
...
app.controller('QuizContentsCtrl', function($scope, $mdDialog, $http, $filter, $location, Global) {
    ...
    $http.get("http://129.237.121.131:3000/quiz").then(function(response) {
    ...
    $scope.toggleActive = function() {
        ...
        $http.put("http://129.237.121.131:3000/quiz/" + $scope.global.tempId, JSON.stringify($scope.dataObj));

        $scope.global.quizzes[$scope.global.currentQuizIndex].isVisible = toggle;
    };

    // GETTING QUIZ QUESTION DATA
    $http.get("http://129.237.121.131:3000/question?quizId=" + $scope.global.currentQuizId).then(function(response) {
       $scope.global.questions = response.data;
    });

    // GETTING SUBMISSION DATA
    $http.get("http://129.237.121.131:3000/submission?quizId=" + $scope.global.currentQuizId).then(function(response) {
       $scope.global = Global;
...
app.controller('AddCourseCtrl', function($scope, $mdDialog, $http, Global) {
    $scope.submit = function() {
        ...
        $http.get("http://129.237.121.131:3000/course").then(function(response) {
            $scope.global = Global;
        ...
        $http.post("http://129.237.121.131:3000/course", $scope.data);
...
app.controller('AddQuizCtrl', function($scope, $mdDialog, $http, Global) {
    $scope.submit = function() {
        ...
        $http.get("http://129.237.121.131:3000/quiz").then(function(response) {
            ...
            $http.post("http://129.237.121.131:3000/quiz", $scope.data);
    ...
    $scope.deleteCourse = function() {
        ...
        $http.get("http://129.237.121.131:3000/course").then(function(response) {
           ...
           for (var i = 0; i < $scope.global.temp.length; i++) {
                ...
                $http.put("http://129.237.121.131:3000/course/" + i, JSON.stringify($scope.newDataObj));
                console.log("JUST PUT AT: " + i);
           }

           $http.delete("http://129.237.121.131:3000/course/" + $scope.global.temp.length);
    ...
    $scope.deleteQuiz = function(index) {
        ...
        $http.get("http://129.237.121.131:3000/quiz").then(function(response) {
            ...
            for (var i = 0; i < $scope.global.temp.length; i++) {
                ...
                $http.put("http://129.237.121.131:3000/quiz/" + i, JSON.stringify($scope.newDataObj));
                console.log("JUST PUT AT: " + i);
            }

            $http.delete("http://129.237.121.131:3000/quiz/" + $scope.global.temp.length);
...
app.controller('AskDeleteQuestionCtrl', function($scope, $mdDialog, $http, $filter, Global) {
    ...
    $scope.deleteQuestion = function(index) {
        ...
        $http.get("http://129.237.121.131:3000/question").then(function(response) {
            $scope.global.temp = response.data;
            ...
            for (var i = 0; i < $scope.global.temp.length; i++) {
                ...
                $http.put("http://129.237.121.131:3000/question/" + i, JSON.stringify($scope.newDataObj));
                console.log("JUST PUT AT: " + i);
            }

            $http.delete("http://129.237.121.131:3000/question/" + $scope.global.temp.length);
...
app.controller('AddTFQuestionCtrl', function($scope, $mdDialog, $http, Global) {
    ...
    $scope.submit = function() {
      ...
      $http.post("http://129.237.121.131:3000/question", $scope.data);
